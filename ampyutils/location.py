...
def locateProg(progName: str, logger: logging.Logger = None):
    """
    ...
    if exePROG is None:
        if logger is not None:
            logger.info('{func:s} !!! did not found executable {prog:s} in path. Program Exits\n'.format(func=cFuncName, prog=colored(progName, 'yellow')))
        else:
            logger.error(colored('!!! did not found executable {prog:s} in path. Program Exits\n'.format(prog=progName), 'red'))
