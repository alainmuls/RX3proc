...
def file_exists(fname: str, logger: logging.Logger = None) -> bool:
    """
    file_exists checks for existence of file
    """
    cFuncName = colored(os.path.basename(__file__), 'yellow') + ' - ' + colored(sys._getframe().f_code.co_name, 'green')

    if not os.path.isfile(fname):
        if logger is not None:
            logger.error('{func:s}: File {name:s} does not exist.'.format(name=colored(fname, 'yellow'), func=cFuncName))
        return False
    else:
        return True
...
def changeDir(directory, verbose=False):
    ...
    if not os.path.exists(workDir):
        if verbose:
            sys.stderr.write('Directory {dir:s} does not exists.\n'.format(dir=workDir))
        return False
    else:
        os.chdir(workDir)
        return True
...
def yeardoy2ymd(year: int, doy: int) -> datetime.date:
    """
    yeardoy2ymd converts YEAR/DOY to datetime.date object
    """
    # converting to date
    # res = datetime.strptime('{year:04d}-{doy:03d}'.format(year=year, doy=doy), "%Y-%j").strftime("%m-%d-%Y")
    # print('res = {!s}  {!s}'.format(res, type(res)))
    return datetime.strptime('{year:04d}-{doy:03d}'.format(year=year, doy=doy), "%Y-%j").date()
    # print('res = {!s}  {!s}'.format(res, type(res)))
...
def run_subprocess(sub_proc: list, logger: logging.Logger = None) -> int:
    """
    ...
    try:
        ...
        subprocess.check_output(strargs, stderr=subprocess.DEVNULL)
        return amc.E_SUCCESS
        ...
        return(amc.E_SBF2RIN_ERRCODE)
    except OSError as e:
        # executable not found
        if logger is not None:
            logger.error('{func:s}: subprocess {proc:s} returned error code {err!s}'.format(func=cFuncName, proc=strargs[0], err=e))
        return(amc.E_OSERROR)


def run_subprocess_output(sub_proc: list, logger: logging.Logger = None) -> Tuple[int, str]:
    """
    run_subprocess runs the program with arguments in the sub_proc list
    """
    cFuncName = colored(os.path.basename(__file__), 'yellow') + ' - ' + colored(sys._getframe().f_code.co_name, 'green')
    ...
    try:
        ...
        if logger is not None:
            logger.error('{func:s}: subprocess {proc:s} returned error code {err!s}'.format(func=cFuncName, proc=strargs[0], err=e))
        return amc.E_SBF2RIN_ERRCODE, None
    except OSError as e:
        # executable not found
        if logger is not None:
            logger.error('{func:s}: subprocess {proc:s} returned error code {err!s}'.format(func=cFuncName, proc=strargs[0], err=e))
        return amc.E_OSERROR, None
...
def txt_lines_with(fname: str, substr: str):
    """
